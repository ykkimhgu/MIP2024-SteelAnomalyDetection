...
def test(obj_names, mvtec_path, checkpoint_path, base_model_name):
    ...
    for obj_name in obj_names:
        ...
        model.load_state_dict(torch.load(os.path.join(checkpoint_path, run_name+ "best" + ".pckl"), map_location='cuda:0'))
        model.cuda()
        model.eval()

        model_seg = DiscriminativeSubNetwork(in_channels=6, out_channels=2)
        model_seg.load_state_dict(torch.load(os.path.join(checkpoint_path, run_name+"_seg_best.pckl"), map_location='cuda:0'))
        model_seg.cuda()
        model_seg.eval()
        ...
        t_mask = torch.zeros((1 ,256 ,256)).cuda()
...
if __name__=="__main__":
    ...
    parser.add_argument('--base_model_name', action='store', type=str, required=False, default="DRAEM_test_0.0001_1500_bs8")
    parser.add_argument('--data_path', action='store', type=str, required=False, default="../datasets/Severstal/")
